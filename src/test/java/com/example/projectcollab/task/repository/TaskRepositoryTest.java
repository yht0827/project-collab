...
class TaskRepositoryTest {
	...
	@Nested
	@DisplayName("searchTasks JPQL 동적 검색 및 페이징 검증")
	class SearchTasksQuery {
		...
		void searchWithoutConditions() {
			...
			Page<Task> result = taskRepository.searchTasks(project.getId(), null, null, pageable);
		...
		void searchByStatus() {
			...
			Page<Task> result = taskRepository.searchTasks(project.getId(), TaskStatus.IN_PROGRESS, null, pageable);
		...
		void searchByKeyword() {
			...
			Page<Task> result = taskRepository.searchTasks(project.getId(), null, "Spring", pageable);
		...
		void searchByStatusAndKeyword() {
			...
			Page<Task> result = taskRepository.searchTasks(project.getId(), TaskStatus.DONE, "AWS", pageable);
	...
	class DeleteAllByProjectId {
		...
		void deleteAllTasksInProject() {
			...
			Page<Task> result = taskRepository.searchTasks(project.getId(), null, null, PageRequest.of(0, 10));
